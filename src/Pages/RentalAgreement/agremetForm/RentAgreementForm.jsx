// File: src/components/RentAgreementForm.jsx
// ============================================
...
const STEP_TITLES = {
  1: 'Agreement Type & Owner Details',
  2: 'Tenant Details',
...
const INITIAL_FORM_DATA = {
  formType: 'rental_agreement', // Default form type
...
const VALIDATION_RULES = {
  1: ['formType', 'state', 'ownerName', 'ownerRelation', 'ownerRelatedName', 'ownerAge', 'ownerAddress', 'ownerPan', 'ownerAadhaar', 'ownerContact', 'ownerPhoto'],
  ...
  4: ['paymentMethod', 'duration', 'commenceDate', 'endDate', 'noticePeriod', 'lockInPeriod', 'defaultPeriod', 'rentIncrement', 'purpose', 'stampDutyBy'],
  5: ['email']
};
...
export default function RentAgreementForm() {
  ...
  const validateStep = (stepNumber) => {
    ...
    requiredFields.forEach(field => {
      if (!isFilled(formData[field])) {
        ...
        if (field.includes('Pan') && !validatePAN(formData[field])) {
  ...
  // Submit Handler - Only API submission, no payment
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.log('📝 Submitting agreement with formType:', formData.formType);
      
      // Submit agreement directly with formType
      const agreementResult = await ApiService.submitAgreement({
  ...
  // Get selected form type label for display
  const selectedFormTypeLabel = FORM_TYPES.find(t => t.value === formData.formType)?.label || 'Agreement';
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-white to-blue-50 p-4 md:p-6">
      <div className="max-w-7xl mx-auto mt-20">
        <div className="text-center mb-6">
          ...
          {formData.formType && (
            <div className="mt-3 inline-block px-4 py-2 bg-purple-100 border-2 border-purple-300 rounded-lg">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
          ...
          <div className="bg-white border-2 border-gray-200 rounded-xl p-6 md:p-8 shadow-xl h-fit">
            ...
            <div className="space-y-5">
              {step === 1 && (
                <>
                  <Input 
                    ...
                    onFocus={() => handleFocus('formType')} 
                    onBlur={handleBlur} 
                    isActive={activeField === 'formType'} 
                    error={validationErrors.formType} 
                  />
              ...
              {step === 3 && (
                <>
                  <Input label="Property Address" name="propertyAddress" rows={2} placeholder="Complete address" value={formData.propertyAddress} onChange={handleChange} onFocus={() => handleFocus('propertyAddress')} onBlur={handleBlur} isActive={activeField === 'propertyAddress'} error={validationErrors.propertyAddress} />
                  <div className="grid grid-cols-2 gap-4">
                  ...
                  <Input label="Maintenance (₹)" name="maintenanceCharges" type="number" placeholder="2000" value={formData.maintenanceCharges} onChange={handleChange} onFocus={() => handleFocus('maintenanceCharges')} onBlur={handleBlur} isActive={activeField === 'maintenanceCharges'} error={validationErrors.maintenanceCharges} />
                ...
                <div className="bg-white p-6 rounded-lg border border-gray-200">
                  <div className="text-center mb-6 pb-4 border-b-2 border-gray-300">
                    <h1 className="text-2xl font-bold text-gray-900 uppercase tracking-wide">
                      {selectedFormTypeLabel.toUpperCase()}
                    </h1>
                  </div>
...
// const VALIDATION_RULES = {
//   1: ['state', 'ownerName', 'ownerRelation', 'ownerRelatedName', 'ownerAge', 'ownerAddress', 'ownerPan', 'ownerAadhaar', 'ownerContact', 'ownerPhoto'],
//   2: ['tenantName', 'tenantRelation', 'tenantRelatedName', 'tenantAge', 'tenantAddress', 'tenantPan', 'tenantAadhaar', 'tenantContact', 'tenantPhoto'],
//   3: ['propertyAddress', 'monthlyRent', 'maintenanceCharges', 'rentPayDay', 'securityDeposit'],
//   4: ['paymentMethod', 'duration', 'commenceDate', 'endDate', 'noticePeriod', 'lockInPeriod', 'defaultPeriod', 'rentIncrement', 'purpose', 'stampDutyBy'],
//   5: ['email']
...
// // Signature Table Component
// // Fixed Signature Table Component - Replace in your RentAgreementForm.jsx
// // ============================================
...
//             <td className="px-3 py-2 font-semibold w-1/3 border-r border-black">{key}:</td>
//             <td className={`px-3 ${key === 'Signature' ? 'py-8' : 'py-2'}`}>
//               {/* Check if value is a photo (base64 string starting with 'data:image') */}
//               {value && typeof value === 'string' && value.startsWith('data:image') ? (
//                 <img 
//                   src={value} 
//                   alt={key} 
...
//               <h2 className="text-xl font-bold">📄 HOUSE RENTAL AGREEMENT</h2>
