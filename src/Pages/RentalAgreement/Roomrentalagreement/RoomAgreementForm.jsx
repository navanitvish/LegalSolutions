// File: src/components/RoomAgreementForm.jsx
// ============================================

import { useState, useRef, useEffect } from 'react';
import {RoomgenerateAgreementText} from '../../../utils/DocumentData';
...
const VALIDATION_RULES = {
  ...
  3: ['propertyAddress', 'monthlyRent', 'monthlyRentWords', 'rentPayDay', 'securityDeposit', 'securityDepositWords'],
  4: ['paymentMode', 'duration', 'commenceDate', 'endDate'],
  5: ['email']
};
...
export default function RoomAgreementForm() {
  ...
  const handleChange = (field, value) => {
    ...
    if (field === 'duration' && parseInt(value) > 11) {
  ...
  const handlePaymentSuccess = async (paymentResponse) => {
    try {
      ...
      const emailResult = await ApiService.sendEmail(formData.email, {
        agreementText: RoomgenerateAgreementText(formData),
  ...
  const agreementText = RoomgenerateAgreementText(formData);
      ...
      <div className="max-w-7xl mx-auto mt-20">
        <div className="text-center mb-6">
          <h1 className="text-3xl md:text-4xl font-bold bg-gradient-to-r from-purple-600 to-blue-600 bg-clip-text text-transparent">
            Room Rental Agreement Generator
          </h1>
          <p className="text-gray-600 mt-2">Create professional room rental agreement in minutes</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
          ...
          <div className="bg-white border-2 border-gray-200 rounded-xl p-6 md:p-8 shadow-xl h-fit">
            ...
            <div className="space-y-5">
              ...
              {step === 3 && (
                <>
                  <Input label="Property Address (Room/Rented Portion)" name="propertyAddress" rows={3} placeholder="Complete address of the room" value={formData.propertyAddress} onChange={handleChange} onFocus={() => handleFocus('propertyAddress')} onBlur={handleBlur} isActive={activeField === 'propertyAddress'} error={validationErrors.propertyAddress} />
                  <div className="grid grid-cols-2 gap-4">
                    <Input label="Area (Sqft)" name="propertyArea" placeholder="e.g., 200" required={false} value={formData.propertyArea} onChange={handleChange} onFocus={() => handleFocus('propertyArea')} onBlur={handleBlur} isActive={activeField === 'propertyArea'} />
                  ...
                  <Input label="Specifications (Optional)" name="propertySpecifications" rows={2} placeholder="Any additional specifications" required={false} value={formData.propertySpecifications} onChange={handleChange} onFocus={() => handleFocus('propertySpecifications')} onBlur={handleBlur} isActive={activeField === 'propertySpecifications'} />
                  <Input label="Monthly Rent (₹)" name="monthlyRent" type="number" placeholder="8000" value={formData.monthlyRent} onChange={handleChange} onFocus={() => handleFocus('monthlyRent')} onBlur={handleBlur} isActive={activeField === 'monthlyRent'} error={validationErrors.monthlyRent} />
                  <Input label="Monthly Rent in Words" name="monthlyRentWords" placeholder="Eight Thousand Only" value={formData.monthlyRentWords} onChange={handleChange} onFocus={() => handleFocus('monthlyRentWords')} onBlur={handleBlur} isActive={activeField === 'monthlyRentWords'} error={validationErrors.monthlyRentWords} />
                  ...
                  <Input label="Security Deposit (₹)" name="securityDeposit" type="number" placeholder="16000" value={formData.securityDeposit} onChange={handleChange} onFocus={() => handleFocus('securityDeposit')} onBlur={handleBlur} isActive={activeField === 'securityDeposit'} error={validationErrors.securityDeposit} />
                  <Input label="Security Deposit in Words" name="securityDepositWords" placeholder="Sixteen Thousand Only" value={formData.securityDepositWords} onChange={handleChange} onFocus={() => handleFocus('securityDepositWords')} onBlur={handleBlur} isActive={activeField === 'securityDepositWords'} error={validationErrors.securityDepositWords} />
                  <Input label="Maintenance Charges (Optional)" name="maintenanceCharges" placeholder="If any" required={false} value={formData.maintenanceCharges} onChange={handleChange} onFocus={() => handleFocus('maintenanceCharges')} onBlur={handleBlur} isActive={activeField === 'maintenanceCharges'} />
                  <Input label="Other Charges (Optional)" name="otherCharges" placeholder="Car parking, society fees etc" required={false} value={formData.otherCharges} onChange={handleChange} onFocus={() => handleFocus('otherCharges')} onBlur={handleBlur} isActive={activeField === 'otherCharges'} />
                </>
              )}

              {step === 4 && (
                <>
                  <Input label="Payment Mode" name="paymentMode" options={['Cheque', 'NEFT', 'Bank Transfer', 'UPI', 'Cash']} value={formData.paymentMode} onChange={handleChange} onFocus={() => handleFocus('paymentMode')} onBlur={handleBlur} isActive={activeField === 'paymentMode'} error={validationErrors.paymentMode} />
                  ...
                  <div>
                    ...
                    {showDurationWarning && (
                      <div className="mt-2 p-4 bg-yellow-50 border-2 border-yellow-400 rounded-lg">
                        <div className="flex items-start gap-3">
                          <span className="text-2xl">⚠️</span>
                          <div className="flex-1">
                            <p className="font-bold text-yellow-800 mb-1">Important Notice</p>
                            <p className="text-sm text-yellow-700">
                              If your period of duration is more than 11 months, it is mandatory to register the rental agreement with the registrar office. It is advised that you register your agreement even if the duration period is less than 11 months.
                            </p>
                          </div>
                        </div>
                      </div>
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-800 mb-3">Utilities/Fittings in Room (Optional):</label>
